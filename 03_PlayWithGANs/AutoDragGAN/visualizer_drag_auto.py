...
class Visualizer(imgui_window.ImguiWindow):
    ...
    def _apply_face_adjustment(self, image, mode):

        # 确保图像为 RGB 格式
        print('Image shape is ', image.shape)
        if image.shape[2] == 4:  # 如果图像有 4 个通道 (RGBA)
            image = image[:, :, :3]  # 只取前三个通道（RGB）
        elif image.shape[2] != 3:
            raise ValueError("输入图像必须是 RGB 格式，包含 3 个通道。")
        ...
        if landmarks is None:
            ...
            return
        landmarks = landmarks[0].astype(int).tolist()  # 使用第一张人脸的特征点
        ...
        if mode == "close_eyes":
            midd_point = [(landmarks[37][0] + landmarks[41][0]) // 2, (landmarks[37][1] + landmarks[41][1]) // 2]
            start_points.extend([landmarks[37], landmarks[41]])  # 左眼
            end_points.extend([midd_point, midd_point])

        elif mode == "big_eyes":
            auxi_line = [landmarks[41][0] - landmarks[37][0], landmarks[41][1] - landmarks[37][1]] 
            intensity = 0.8
            end_1 = [math.ceil(landmarks[37][0] - intensity * auxi_line[0]), math.ceil(landmarks[37][1] - intensity * auxi_line[1])]
            end_2 = [math.ceil(landmarks[41][0] + intensity * auxi_line[0]), math.ceil(landmarks[41][1] + intensity * auxi_line[1])]
            start_points.extend([landmarks[37], landmarks[41]])
            end_points.extend([end_1, end_2])

        elif mode == "shut_mouth":
            midd_point = [(landmarks[62][0] + landmarks[66][0]) // 2, (landmarks[62][1] + landmarks[66][1]) // 2]
            start_points.extend([landmarks[62], landmarks[66]])  # 嘴巴
            end_points.extend([midd_point, midd_point])

        elif mode == "open_mouth":
            start_points.extend([landmarks[62], landmarks[66]])
            end_points.extend([landmarks[51], landmarks[57]])

        elif mode == "slim_face":
            auxi_line = [landmarks[15][0] - landmarks[1][0], landmarks[15][1] - landmarks[1][1]]
            intensity = 0.1
            end_1 = [math.ceil(landmarks[1][0] + intensity * auxi_line[0]), math.ceil(landmarks[1][1] + intensity * auxi_line[1])]
            end_2 = [math.ceil(landmarks[15][0] - intensity * auxi_line[0]), math.ceil(landmarks[15][1] - intensity * auxi_line[1])]
            start_points.extend([landmarks[1], landmarks[15]])
            end_points.extend([end_1, end_2])

        # 将多对点反转，以适应 Drag 功能
        start_points_rev = [sublist[::-1] for sublist in start_points]
        end_points_rev = [sublist[::-1] for sublist in end_points]
        # 将多对点传递给 Drag 功能
        self.drag_widget.points = start_points_rev
        self.drag_widget.targets = end_points_rev

        # self.drag_widget.is_drag = True            # 启动拖拽
    ...
    def draw_frame(self):
        ...
        # 添加面部调整按钮
        expanded, _visible = imgui_utils.collapsing_header('Face Adjustments', default=True)
        if expanded:
            if imgui.button("Close Eyes"):
                self.adjustment_mode = "close_eyes"
            if imgui.button("Big Eyes"):
                self.adjustment_mode = "big_eyes"
            if imgui.button("Shut Mouth"):
                self.adjustment_mode = "shut_mouth"
            if imgui.button("Open Mouth"):
                self.adjustment_mode = "open_mouth"
            if imgui.button("Slim Face"):
                self.adjustment_mode = "slim_face"

        # 当按钮被点击时，应用相应的面部调整
        if self.adjustment_mode and "image" in self.result:
            # 从当前显示图像中获取面部特征点
            self._apply_face_adjustment(self.result.image, self.adjustment_mode)
            self.adjustment_mode = None  # 重置模式
