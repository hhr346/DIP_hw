...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    Args:
        foreground_img (torch.Tensor): Foreground image tensor.         [1,3,506,416]
        foreground_mask (torch.Tensor): Foreground mask tensor.         [1,1,506,416]
    ...
    laplacian_kernel = torch.tensor([[0, 1, 0],
                                    [1, -4, 1],
                                    [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # (1, 1, 3, 3)

    # 扩展拉普拉斯核以适用于 RGB 图像（3 通道）
    laplacian_kernel = laplacian_kernel.expand(3, 1, 3, 3).to(foreground_img.device) # (3, 1, 3, 3)

    # 假设 img 是一个 RGB 图像 (1, 3, H, W)
    def compute_laplacian_rgb(img):
        # 使用卷积操作对每个通道分别计算二阶导数（拉普拉斯算子）
        laplacian = F.conv2d(img, laplacian_kernel, padding=1, groups=3)  # 使用 groups=3 处理每个通道
        return laplacian
    
    # 计算前景图像和混合图像的拉普拉斯损失
    foreground_laplacian = compute_laplacian_rgb(foreground_img)
    blended_laplacian = compute_laplacian_rgb(blended_img)

    foreground_mask_bool = (foreground_mask > 0)
    background_mask_bool = (background_mask > 0)
    torch.set_printoptions(threshold=torch.inf)  # 设置阈值为无穷大，以显示所有元素

    foreground_mask_bool = foreground_mask_bool.expand(-1, 3, -1, -1)
    background_mask_bool = background_mask_bool.expand(-1, 3, -1, -1)

    # 计算对应掩码下的差值
    a_valid = foreground_laplacian[foreground_mask_bool]
    b_valid = blended_laplacian[background_mask_bool]

    difference = a_valid - b_valid
